...
class Cart extends CI_Controller {
    ...
    public function __construct(){
		...
		$pickupstore = $this->db_model->get_row('settings',array('key'=>'shipping-pickupStore'))->value;
		if($this->session->userdata('pickupstore') != null)
				{
				$this->session->set_userdata('shipping_charges',$pickupstore);
				$this->session->set_userdata('shipping_type','Pickup From The Store');
				}
	...
	public function update(){
		if($this->input->post())
		{
			...
			$this->session->set_userdata('pickupstore',$vals['pickupstore']);
	...
	function sendConfirmation($order_id){
			...
			$this->email->initialize($config);
			$toadmin = $this->db_model->get_admin_email();
			$nameuser = $this->session->userdata('member_data')->first_name." ".$this->session->userdata('member_data')->last_name;
			$subjectadmin = 'OrderId'.$order->id.' From '.$nameuser;
			$this->email->from($to, $nameuser);
			$this->email->to($toadmin);
			$this->email->subject($subjectadmin);
			$this->email->message($body);
			$this->email->send();	
			//echo $this->email->print_debugger();
