...
class giftcard extends CI_Controller {
	...
	public function index()
	{
        if($this->input->post() && $this->validate())
        {
			...
			$vals['subject'] = stripslashes($vals['subject']);
            $gvals['member_id'] = 0;
            $gvals['purchase_date'] = date('Y-m-d h:i:s');
            $gvals['code'] = $vals['code'];
	...
	{
		$query = "SELECT code,amount,purchase_date,email,redeemed,(select concat(first_name,' ',last_name) from {PRE}members where id = {PRE}gift_cards.redeemed_by) as redeemed_by,redeem_date, id as ed FROM {PRE}gift_cards";
   ...
   private function rand_string( $length ) {
	...
	public function del($id = 0)
	{
		...
		$res = $this->db_model->delete_row("gift_cards",array('id'=>$id));
	...
	{
		$res = $this->db_model->get_row('gift_cards',array('id'=>$id));
