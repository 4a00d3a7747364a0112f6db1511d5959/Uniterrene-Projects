...
class Member extends CI_Controller {
    ...
    public function signup(){
	    ...
	    if($this->input->post()){
			...
			//print_r($result); exit;	
				...
				//// notify admin
				$em_data = array('user_name' =>$this->input->post('first_name').' '.$this->input->post('middle_name').' '.$this->input->post('last_name'),'userid'=>$userID);
				$this->send_notification_to_admin($em_data);
				//exit;
				...
				{
					//strip shlashes
					$green_id_data = $this->db_model->remove_slashes($green_id_data);
					...
					$activation_data = array('user_name' =>$this->input->post('first_name').' '.$this->input->post('middle_name').' '.$this->input->post('last_name'),
											 'email' => $this->input->post('email'),
											 'activation_code' => $activation_code,'userid'=>$userID);
					
					$activation_data = $this->db_model->remove_slashes($activation_data);
	...
	private function send_notification_to_admin($vals){

		$link_url = base_url()."administration/members/edit/".$vals['userid'];
        $to = $this->db_model->get_admin_email();
        $email_temp = $this->db_model->get_row('email_templates',array('id'=>8));
        $subject = $email_temp->subject;
        $body = $email_temp->content;
        $body = str_replace('{name}',$vals['user_name'],$body);
        $body = str_replace('{link}',$link_url,$body);
    ...
    private function create_session($email='',$password='')
	{
        ...
        if($mem_data)
		{
			...
			else if($mem_data->status == "Active")
			{
				...
				if($mem_data->greenid_verified =="Active")
				{}
				else
				{
						...
						if($outcome!='' && ($outcome == 'VERIFIED' || $outcome == 'VERIFIED_ADMIN' || $outcome =='VERIFIED_WITH_CHANGES'))
						{
							$this->db_model->update_row('members',array('greenid_verified'=>'Active','greenid_status'=>1),array('id'=>$this->	session->userdata('member_data')->id));
							
							
							$mem_data1 = $this->db_model->get_row("members",array('id'=>$this->session->userdata('member_data')->id));
							
							$this->session->set_userdata('member_data',$mem_data1);
							
					}
					else
					{
						$this->db_model->update_row('members',array('greenid_verified'=>'Inactive','greenid_status'=>0),array('id'=>$this->session->userdata('member_data')->id));
    ...
    public function myaccount()
	{
		...
		if($member_data->greenid_verified == 'Inactive')
		{
	...
	public function order($id='')
	{
        ...
        $data['page_view'] = "web/pages/pg-myaccount-order";
