...
{?>
              <tr>
                <td colspan="3" align="right" class="txt19"><?php echo $this->session->userdata('shipping_type'); ?> (Non Metro):&nbsp;&nbsp;</td>
                <td class="txt19"><?PHP echo $this->config->item('currency_symbol')."&nbsp;".$this->session->userdata('shipping_charges')*2; //$this->helper_model->format_currency($shipping_charges);//echo $this->config->item('currency_symbol')."&nbsp;".$this->helper_model->calculate_shipping($this->helper_model->calculate_gst($shipping_charges),$shipping_charges);
				$shipping_chargeszip = $this->session->userdata('shipping_charges')*2;
				?></td>
              </tr>
           <?php
}else{
	?>
      <tr>
                <td colspan="3" align="right" class="txt19"><?php echo $this->session->userdata('shipping_type'); ?> :&nbsp;&nbsp;</td>
                <td class="txt19"><?PHP echo $this->config->item('currency_symbol')."&nbsp;".$this->session->userdata('shipping_charges'); //$this->helper_model->format_currency($shipping_charges);//echo $this->config->item('currency_symbol')."&nbsp;".$this->helper_model->calculate_shipping($this->helper_model->calculate_gst($shipping_charges),$shipping_charges);?></td>
              </tr>
    <?php
	$shipping_chargeszip = $this->session->userdata('shipping_charges');
	}
	$shipping_charges = $shipping_chargeszip;
?>   
  ...
  <tr>
    <td style="border-top: 1px dotted #ccc;"><h2>More gear you'll love...</h2>
      <style>
                        .product-listingDiv
                        {
                            margin-left: 0%;
      ...
      <div class="middle-l-f-items-1">
        ...
        <div class="product-listingDiv">
          <div class="f-items-grey-bg3" style="border-bottom: 1px solid #f0f0f0;">
            <div class="middle-l-f-items-img"> <a href="<?PHP echo $this->product_model->gen_product_url($product->id);?>"> <img alt="<?PHP echo $caption;?>" src="<?PHP echo $image;?>" border="0" /> </a> </div>
          </div>
