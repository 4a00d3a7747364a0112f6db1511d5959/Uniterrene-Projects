                        ...
                        <table width="95%" border="0" cellspacing="0" cellpadding="3">
                          ...
                          /*  $max_shipping_charges = max ($s_charges);
							...
							*/
							...
							{
								$shipping_chargeszip = $shipping_charges*2;	
							}
...
?>
                          ...
                          <tr>
                            <td align="left">&nbsp;</td>
                            <td>&nbsp;</td>
                            <td align="right" class="txt19"> Postcode based Shipping Calculation:&nbsp;&nbsp;</td>
                            <td class="txt19"><?php
                            if($this->session->userdata('shippingzipcode') != null && $this->session->userdata('zipcodenonmetro') != null)
{ 
	$shippingzipcode = $this->session->userdata('shippingzipcode');
	$zipcodenonmetro = $this->session->userdata('zipcodenonmetro');
	
}else{
	$shippingzipcode = '';
	$zipcodenonmetro ='';
	}?>
                              <input type="text" placeholder="Enter Post Code" name="shippingzipcode" id="shippingzipcode" value="<?php echo $shippingzipcode;?>" />
                              <input type="hidden" name="zipcodenonmetro" id="zipcodenonmetro" value="<?php echo $zipcodenonmetro;?>" />
                              <a class="txt16" id="a4_up" href="#">(?)</a>
                              <div id="tip4_up" style="display:none;" class="txt15">Enter
                                your delivery postcode
                                <?php /*?>Premium added for non metro = 
                              double the standard shipping<br />  <?php echo $this->config->item('currency_symbol').$standard_shipp*2;?> Shipping Fee is Charged To You<?php */?>
                              </div></td>
                          </tr>
                          <tr>
                            <td align="left">&nbsp;</td>
                            <td>&nbsp;</td>
                            <td align="right" class="txt19">Pickup From The Store</td>
                            <td class="txt19"><input type="checkbox" value="<?php echo $this->db_model->get_row('settings',array('key'=>'shipping-pickupStore'))->value;?>" name="pickupstore" <?php if($this->session->userdata('pickupstore') != null){?>checked="checked"<?php }?> />
                              <a class="txt16" id="a5_up" href="#">(?)</a>
                              <div id="tip5_up" style="display:none;" class="txt15"> Select this option if you have arranged to pickup from us</div></td>
                          </tr>
                          <tr>
                            <td align="left">&nbsp;</td>
                            <td>&nbsp;</td>
                          ...
                          <tr>
                            ...
                            <td class="txt19" id="totaltd"><?PHP 
							
if($this->session->userdata('shippingzipcode') != null && $this->session->userdata('zipcodenonmetro') != null && $this->session->userdata('zipcodenonmetro') == 'nonmetro')
{ 
echo $this->config->item('currency_symbol')."&nbsp;".$this->helper_model->calculate_total($sub_total,$shipping_chargeszip);
}else{
							echo $this->config->item('currency_symbol')."&nbsp;".$this->helper_model->calculate_total($sub_total,$shipping_charges);
}
                          ...
                          <tr>
							...
							if($this->session->userdata('shippingzipcode') != null && $this->session->userdata('zipcodenonmetro') != null && $this->session->userdata('zipcodenonmetro') == 'nonmetro')
{ 
									$total = $this->helper_model->calculate_total($sub_total,$shipping_chargeszip);
}else{
    ...
    function cal_no_of_days(i){
		...
		if(secondDate < firstDate) {
			...
			else
			{
				...
				if(diffDays < 3){ 
				//alert('Minimal rental is 3 days.');
				//document.getElementById('return_date'+i+'').value = '';
				}
				if(diffDays == 0){ 
				//alert('Minimal rental is 3 days.');
