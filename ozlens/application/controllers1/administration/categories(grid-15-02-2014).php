...
class Categories extends CI_Controller {
    ...
    protected function getCategoryTree($level = 0, $padding = 0) {
        $rows = $this->db
            ->select('*')
            ->where('p_id', $level)
        ...
        if (count($rows) > 0)
        {
            foreach ($rows as $row) {
                ...
                //$category .= $padding . $row->cat_name . "<br/>";
                // Append subcategories
               // $category .= $this->getCategoryTree($row->id, $padding . '-');

                $indx_id = $row->id;

                if($this->categories_model->hasChild($row->id))
                {
                    $cat_name = "<span style='padding-left:".$padding."px;font-weight:bold;'>". $row->cat_name."</span>";
                }
                else
                {
                    $cat_name = "<span style='padding-left:".$padding."px;'>". $row->cat_name."</span>";
                }


                $edit_url = "<a href='".base_url()."administration/categories/edit/".$indx_id."'><img src='".base_url()."assets/images/administration/icons/edit.gif'/></a>";

                $del_url = "<a  href='".base_url()."administration/categories/del/".$indx_id."' onclick='return confirm(\\\"Are you sure you want to continue?\\\");'><img src='".base_url()."assets/images/administration/icons/del.gif'/></a>";

                $options = $edit_url." | ".$del_url;


                $category .= '["'.$cat_name.'","'.$row->status.'","'.date("F j, Y, g:i a",strtotime($row->last_modified)).'","'.$options.'"],';
                $category .= $this->getCategoryTree($row->id, ($padding+20));
            }
        }
    ...
    public function grid($p_id = 0)
    {
        echo "{ \"aaData\": [";
        echo trim($this->getCategoryTree($p_id), ",");
        echo "] }";
    }
