...
/*------------------------------------------------------------------------------+
| MagneticOne                                                                   |
| Copyright (c) 2017 MagneticOne.com <contact@magneticone.com>                  |
| All rights reserved                                                           |
+-------------------------------------------------------------------------------+
...
class M1_Pdo
{
  public $config = null; // config adapter
  public $result = array();
  public $noResult = array('delete', 'update', 'move', 'truncate', 'insert', 'set', 'create', 'drop', 'replace', 'alter', '/*');
  ...
  public function connect()
  {
    ...
    while ($triesCount) {
      try {
        $link = new PDO("mysql:host=$host;$port dbname=$dbName", $this->config->userName, $password);
...
class M1_Mysqli
{
  ...
  public function query($sql, $fetchType)
  {
    ...
    if (isset($_GET['disable_checks'])) {
      mysqli_query($dataBaseHandle, 'SET @OLD_FOREIGN_KEY_CHECKS=@@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS=0');
      mysqli_query($dataBaseHandle, "SET @OLD_SQL_MODE=@@SQL_MODE, SQL_MODE='NO_AUTO_VALUE_ON_ZERO'");
    }
    ...
    if (isset($_GET['disable_checks'])) {
      mysqli_query($dataBaseHandle, "SET SQL_MODE=IFNULL(@OLD_SQL_MODE,'')");
      mysqli_query($dataBaseHandle, "SET FOREIGN_KEY_CHECKS=IFNULL(@OLD_FOREIGN_KEY_CHECKS,0)");
    }
...
class M1_Bridge_Action_Clearcache
{
  ...
  public function magentoClearCache()
  {
    ...
    if ($phpExecutable) {
      // MAGENTO 2.X
      if (file_exists(M1_STORE_BASE_DIR . 'app/etc/env.php')) {
        //we should wait for that process
        @exec($phpExecutable . " bin/magento indexer:reindex > /dev/null");
        @exec($phpExecutable . " bin/magento cache:flush > /dev/null");
        @exec($phpExecutable . " bin/magento catalog:images:resize > /dev/null");
        $this->_removeDirRec('var/cache', false);
      } else {
